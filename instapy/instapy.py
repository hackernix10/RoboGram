...
class InstaPy:
    ...
    def like_follow_comment_by_tags_unfollow_by_list(self,
                     tags=None,
                     amount=50,
                     follow_number=0,
                     comment_number=0,
                     ...
                     skip_top_posts=True,
                     session_key=0):
        """Likes images and follows per tags and unfollows users in list"""
        if self.aborting:
            return self
        ...
        # try to load stats
        session_file = open("logs/session_stats.txt", "r")
        session_stats = (session_file.readline()).split()
        session_file.close()
        if (session_key == int(session_stats[0])):
            liked_img = int(session_stats[1])
            commented = int(session_stats[3])
            followed = int(session_stats[2])
        # file for loging follows
        followed_file = open("logs/list_followed.txt", "r")
        followed_users = []
        for line in followed_file:
            if (line != '\n'):
                if(line[len(line)-1] == '\n'):
                    followed_users.append(line[:len(line)-1]) 
                else:
                    followed_users.append(line)
        followed_file.close()
        ...
        # main loop
        while (liked_img < amount or followed < follow_number or 
               commented < comment_number):
            print("Made work:")
            print('Liked {} pictures'.format(liked_img))
            print('Followed {} users'.format(followed))
            print('Commented {} posts'.format(commented))
            ...
            if (choise_op == 1):
                # like an image
                if (liked_img < amount):
                    choise = randint(0, len(tags)-1)
                    print('Like by --> {}'.format(tags[choise].encode('utf-8')))
                    nmbr = randint(1, 5)
                    print('Liking {} picture'.format(nmbr))
                    ...
                    for i, link in enumerate(links):
                        ...
                        try:
                            ...
                            if not inappropriate:
                                liked = like_image(self.browser)
                                liked_img += 1
                                #edit file with stats
                                session_file = open("logs/session_stats.txt", "w")
                                session_file.write(str(session_key) + " " + str(liked_img) + " " + 
                                                   str(followed) + " " + str(commented))
                                session_file.close()
            ...
            #--------------------------------------------------------------------            
                ...
                # follow a user + unfollow from list
                if (followed < follow_number):
                    #first unfollow a user from list
                    print('List of followed users --> {}'.format(followed_users))
                    print('Number followed users --> {}'.format(len(followed_users)))
                    if (followed < len(followed_users)):
                        unfollow_given_user(self.browser, followed_users[followed])
                    #then follow some other one
                    choise = randint(0, len(tags)-1)
                    print('Follow by --> {}'.format(tags[choise].encode('utf-8')))
                    ...
                    for i, link in enumerate(links):
                        ...
                        try:
                            ...
                            if not inappropriate:
                                ...
                                if (user_name not in self.dont_include and
                                    checked_img and
                                    self.follow_restrict.get(user_name, 0) <
                                        self.follow_times):
                                    followed += follow_user(self.browser,
                                                        ...
                                                        self.username,
                                                        user_name)
                                    if(followed - 1 < len(followed_users)):
                                        followed_users[followed - 1] = user_name;
                                    else:
                                        followed_users.append(user_name)
                                    #edit file with follows
                                    followed_file = open("logs/list_followed.txt", "w")
                                    for j in followed_users:
                                        followed_file.write("%s\n" % j);
                                    followed_file.close()
                                    #edit file with stats
                                    session_file = open("logs/session_stats.txt", "w")
                                    session_file.write(str(session_key) + " " + str(liked_img) + " " + 
            ...
            #--------------------------------------------------------------------                
                ...
                #comment an image + like it
                if (commented < comment_number and 
                    (timer_on == False or (timer_on == True and int(time.time() - start) > 30) )):
                    timer_on = False
                    choise = randint(0, len(tags)-1)
                    print('Comment by --> {}'.format(tags[choise].encode('utf-8')))
                    nmbr = randint(1, 5)
                    print('Commenting {} picture'.format(nmbr))
				    ...
				    #liked = like_image(self.browser)
                                    ...
                                    print('--> Not commented')
                                    sleep(1)
                                # check if commented successfully
                                comment_elem = self.browser.find_elements_by_xpath(
                                    "//a[text()='" + self.username + "']")
                                if (len(comment_elem) < 1):
                                    print("Probably too much comments wait for 30 seconds before commenting again")
                                    timer_on = True
                                    start = time.time()
                                    commented -= 1
                                    break
